...
list scan_update_inventory() {
    ...
    // Scan scripts (exclude self, coordinator, and activator - they're transferred last)
    integer count = llGetInventoryNumber(INVENTORY_SCRIPT);
    integer i = 0;
    while (i < count) {
        string name = llGetInventoryName(INVENTORY_SCRIPT, i);
        // Skip self, coordinator, and activator
        if (name != script_name && 
...
send_coordinator() {
    llOwnerSay("Transferring update infrastructure...");
    ...
    // Transfer activator first
    if (llGetInventoryType("ds_collar_activator_shim") == INVENTORY_SCRIPT) {
        llGiveInventory(CollarKey, "ds_collar_activator_shim");
        llSleep(1.0);
    }
    else {
        llOwnerSay("WARNING: Activator shim not found!");
    }
    
    // Then transfer coordinator
    if (llGetInventoryType("ds_collar_updater_coordinator") == INVENTORY_SCRIPT) {
        ...
        llOwnerSay("Update infrastructure sent. Hot-swap will begin automatically.");
    }
    else {
