/* =============================================================================
   MODULE: ds_collar_menu.lsl (v1.0 - UI/Kmod Split)

   ROLE: Menu rendering and visual presentation (stateless rendering service)

   ARCHITECTURE: Stateless rendering service
   - Receives render requests from ds_collar_kmod_ui
   - Constructs dialogs with proper button layout
   - Handles visual presentation only (no business logic)

   CHANNELS:
   - 900 (UI_BUS): Render requests from UI kmod
   - 950 (DIALOG_BUS): Dialog display

   DESIGN PHILOSOPHY: "Do one thing and do it well"
   - UI kmod handles: Sessions, ACL, plugin filtering, state management
   - Menu handles: Dialog construction, button layout, visual display

   BENEFITS:
   - Separation of concerns: logic vs presentation
   - Memory optimization: reduced per-script complexity
   - Maintainability: single clear responsibility
