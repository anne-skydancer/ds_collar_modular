...
integer PLUGIN_MIN_ACL = 1;  // Level 1 (Public) and above
string ROOT_CONTEXT = "core_root";

list ALLOWED_ACL_GRAB  = [1, 3, 4, 5];     // Public, Trustee, Unowned, Owner (NOT Owned wearer)  // Clip button access
...
show_main_menu() {
    ...
    // Build buttons in reverse order due to show_menu reversal
    // Display order will be: Back, action buttons, Get Holder, Settings
    
    // Settings (rightmost after reversal)
    if (in_allowed_list(UserAcl, ALLOWED_ACL_SETTINGS)) buttons += ["Settings"];
    
    // Get Holder (middle-right after reversal)
    buttons += ["Get Holder"];
    
    // Action buttons (middle after reversal)
    if (!Leashed) {
        if (UserAcl == 2) buttons += ["Offer"];  // Level 2 can offer when NOT leashed
        if (in_allowed_list(UserAcl, ALLOWED_ACL_GRAB)) buttons += ["Clip"];
    }
    else {
        if (CurrentUser == Leasher) {
            // Current leasher gets full control
            buttons += ["Yank", "Pass", "Unclip"];
        }
        else if (UserAcl == 2) {
    ...
    // Back button (leftmost after reversal)
    buttons += ["Back"];
...
logd("Button: " + button + " in context: " + MenuContext);
    if (MenuContext == "main") {
        if (button == "Clip") { if (in_allowed_list(UserAcl, ALLOWED_ACL_GRAB)) { send_leash_action("grab"); cleanup_session(); } }
        else if (button == "Unclip") { send_leash_action("release"); cleanup_session(); }
        ...
        else if (button == "Get Holder") { give_holder_object(); show_main_menu(); }
