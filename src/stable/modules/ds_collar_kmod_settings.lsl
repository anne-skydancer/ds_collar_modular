/* =============================================================
   MODULE:  ds_collar_kmod_settings.lsl (OPTIMIZED)
   CHANGES: 
   - O(n²) sanitize algorithm → O(n)
   - Removed wrapper functions (list_has_str, list_contains)
   - Direct use of llListFindList with ~ operator
   ============================================================= */
...
integer kv_list_add_unique(string key_str, string elem) {
    ...
    if (~llListFindList(L, [ elem ])) return FALSE;
    integer len = llGetListLength(L);
    if (len >= MaxListLen) return FALSE;
    L += [ elem ];
...
// OPTIMIZED: Removed wrapper functions, use llListFindList directly
...
list list_unique(list L) {
    ...
    while (i < n) {
        string s = llList2String(L, i);
        // OPTIMIZED: Direct use
        if (!~llListFindList(U, [s])) U += [s];
        i += 1;
    }
    return U;
}

/* CRITICAL OPTIMIZATION: O(n²) → O(n) */
string sanitize_roles_in_kv(string in_kv) {
    ...
    // OPTIMIZED: Single-pass algorithm instead of O(n²)
    // Build a combined "blocked" list to check against
    list blocked = blacklist;
    if (owner_key != NULL_KEY) {
        blocked += [(string)owner_key];
    }

    // Single pass through trustees, checking against blocked
    list clean_trustees = [];
    integer tr_len = llGetListLength(trustees);
    integer bl_len = llGetListLength(blocked);
    integer i = 0;
    
    // OPTIMIZATION: Cache list length (called once, not in loop)
    while (i < tr_len) {
        string t = llList2String(trustees, i);
        // OPTIMIZED: Direct llListFindList instead of wrapper
        if (!~llListFindList(blocked, [t])) {
            clean_trustees += [t];
        }
        i += 1;
    }
    ...
    if (owner_key != NULL_KEY) {
        // OPTIMIZED: Direct use
        if (~llListFindList(blacklist, [(string)owner_key])) {
            owner_key = NULL_KEY;
        }
    }
            ...
            if (json_has(str, ["values"])) {
                ...
                if (is_json_arr(arr)) {
                    ...
                    if (key_str == KEY_TRUSTEES) {
                        ...
                        integer i = 0;
                        integer L_len = llGetListLength(L);
                        while (i < L_len) {
