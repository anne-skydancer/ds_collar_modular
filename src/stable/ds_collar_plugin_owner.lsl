...
list NameCache;
list PendingNameQueries;  // [qid1, key1, qid2, key2, ...]
list NameRequestQueue;    // Keys waiting to be requested
integer MAX_PENDING_QUERIES = 3;  // Conservative limit for simultaneous dataserver requests
...
string get_name(key k) {
    ...
    // Check if already pending or queued
    if (llListFindList(PendingNameQueries, [k]) == -1 && 
        llListFindList(NameRequestQueue, [k]) == -1) {
        // Add to queue
        NameRequestQueue += [k];
    ...
    // Return legacy name as placeholder
    return llKey2Name(k);
}

// Process queued name requests while respecting throttle limits
process_name_queue() {
    while (llGetListLength(NameRequestQueue) > 0 && 
           llGetListLength(PendingNameQueries) < (MAX_PENDING_QUERIES * 2)) {
        key k = (key)llList2String(NameRequestQueue, 0);
        NameRequestQueue = llDeleteSubList(NameRequestQueue, 0, 0);
        
        // Make the request
        key qid = llRequestDisplayName(k);
        PendingNameQueries += [qid, k];
    }
}

// Pre-cache names for all owners to ensure dataserver resolution
precache_owner_names() {
    if (MultiOwnerMode) {
        integer i = 0;
...
apply_settings_sync(string msg) {
    ...
    // Proactively cache all owner and trustee names
    precache_owner_names();
    integer i = 0;
    integer count = llGetListLength(TrusteeKeys);
    while (i < count) {
        get_name((key)llList2String(TrusteeKeys, i));
        i++;
    }
}
...
persist_owner(key owner, string hon) {
    ...
    // Proactively cache the owner's display name
    if (owner != NULL_KEY) get_name(owner);
}
...
add_trustee(key trustee, string hon) {
    ...
    // Proactively cache the trustee's display name
    get_name(trustee);
}
...
show_main() {
    ...
    // Pre-cache owner names to trigger dataserver resolution for next time
    precache_owner_names();
    ...
    if (has_owner()) {
        if (MultiOwnerMode) {
            ...
            if (owner_count == 1) {
                // Single owner in multi-owner mode
                key owner = (key)llList2String(OwnerKeys, 0);
                string honorific = "";
                if (llGetListLength(OwnerHonorifics) > 0) {
                    honorific = llList2String(OwnerHonorifics, 0);
                }
                body += "Owner: ";
                if (honorific != "") body += honorific + " ";
                body += get_name(owner) + "\n";
            }
            else {
                // Multiple owners
    ...
    if (CurrentUser == llGetOwner()) {
        ...
        else if (RunawayEnabled) buttons += ["Runaway"];
    }
...
default {
    ...
    dataserver(key qid, string data) {
        ...
        integer idx = llListFindList(PendingNameQueries, [qid]);
        if (idx == -1) return;
        
        // Get the associated key and cache the name
        key target = (key)llList2String(PendingNameQueries, idx + 1);
        if (data != "" && data != "???") cache_name(target, data);
        
        // Remove this query from pending list
        PendingNameQueries = llDeleteSubList(PendingNameQueries, idx, idx + 1);
