# AGENTS.md — LSL Technical Requirements & Coding Constraints (Stable Branch)

> A practical reference for agents and assistants that generate or review **Linden Scripting Language (LSL)** code for the **Stable (Production) Branch**. This document covers TECHNICAL REQUIREMENTS ONLY. For style, documentation, and best practices, see `agents-style.md`.
...
5. **CHECK: You are in the `src/stable/` directory - use Stable Plugin ABI (Section 22)**
...
**⚠️ WARNING: This file is for the `src/stable/` directory. Do not mix with Experimental/NG ABI.**
...
Send via:
```lsl
llMessageLinked(LINK_THIS, K_PLUGIN_REG_REPLY, json_payload, NULL_KEY);
```

### 22.3 UI Contract

- **Back** from plugin root → **main UI** (send `plugin_return` message with kernel number).
...
llMessageLinked(LINK_THIS, K_PLUGIN_RETURN_NUM, msg, NULL_KEY);
```
...
**Requesting ACL:**
```lsl
string req = llJsonSetValue("{}",
    ["type"], "auth_request",
    ["context"], PLUGIN_CONTEXT,
    ["avatar"], (string)user
);
llMessageLinked(LINK_THIS, AUTH_QUERY_NUM, req, NULL_KEY);
```

**Handling ACL response:**
```lsl
link_message(integer sender, integer num, string str, key id) {
    if (num == AUTH_RESULT_NUM) {
        integer acl_level = (integer)llJsonGetValue(str, ["level"]);
        integer is_wearer = (integer)llJsonGetValue(str, ["is_wearer"]);
        // Use acl_level to gate features
    }
}
```
...
link_message(integer sender, integer num, string str, key id) {
    if (num == K_PLUGIN_PING) {
        string ping_context = llJsonGetValue(str, ["context"]);
        if (ping_context == PLUGIN_CONTEXT) {
            string pong = llJsonSetValue("{}",
                ["type"], "pong",
                ["context"], PLUGIN_CONTEXT
            );
            llMessageLinked(LINK_THIS, K_PLUGIN_PONG, pong, NULL_KEY);
        }
    }
}
```
...
**Document Version:** 2.0 (Stable Branch)
